...
def smile(mouth):
    ...
    C = dist.euclidean(mouth[5], mouth[8])
    L = (A+B+C)/3
    if (L<15):
        return "closed"
    else:
...
try:
    ...
    frame = vs.read()
    if frame is None:
        print("[ERROR] Failed to access camera. Please check your camera permissions.")
        print("On macOS, go to System Preferences > Security & Privacy > Privacy > Camera")
        print("and ensure Terminal or Python has permission to access the camera.")
        vs.stop()
        sys.exit(1)
    ...
    # compute the convex hull for the left and right eye, then
            ...
            if center_case == True:
                    cv2.putText(frame, "Face in center", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2) 
            ...
            cv2.putText(frame, "eyes: {}".format(eye_case), (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            ####mouth state###
            if mouth_case == "closed" :
                mouth_COUNTER += 1
                #print("hello ")
            else:
                if mouth_COUNTER >= 15:
                    ...
                    mouth_COUNTER = 0
            cv2.putText(frame, "Mouth: {}".format(mouth_case), (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2) 
    ...
    print("If the error is related to camera access, please check your permissions.")
    print("On macOS, go to System Preferences > Security & Privacy > Privacy > Camera")
    print("and ensure Terminal or Python has permission to access the camera.")
